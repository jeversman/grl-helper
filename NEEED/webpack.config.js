...
var isStage = ENV === 'build-stage';
var isProd = ENV === 'build-prod';

var url = isStage ?
  'https://native-herbs-staging.games-project.com:64503'
  : (isProd) ? 'https://native-herbs.games-project.com:64503'
  : 'http://localhost:27781';

var isProd = isProd || isStage; //now configs are same.
...
module.exports = function makeWebpackConfig() {
  ...
  config.entry = isTest ? {} : {
    ...
    'app': root('src/bootstrap.ts') // our angular app
  };
  ...
  config.module = {
    ...
    loaders: [
      ...
      //{test: /require.resolve("jquery")/, loader: 'expose?jQuery!expose?$!expose?jquery'},

      // copy those assets to output
      //{test: /\.(png|jpe?g|gif|svg|woff|woff2|ttf|eot|ico)$/, loader: 'file?name=[path][name].[ext]?[hash]'},
      { test: /\.(eot|woff|woff2|ttf|svg|png|jpe?g|gif)(\?\S*)?$/, loader: 'url?limit=100000&name=[name].[ext]?[hash]'
      },
